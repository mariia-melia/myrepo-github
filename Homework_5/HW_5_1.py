...
for key, value in car_data.items():
  ...
  if year >= search_criteria[0] and engine >= search_criteria[1] and price <= search_criteria[2]:
    final_result[key] = value #adding filtered data into new dict 'final_result'

# Sorting cars by price ascending
sorted_final_result = dict(sorted(final_result.items(), key=lambda item: item[1][4]))

# Print up to five (5) first found elements from 'sorted_final_result'
first_5 = dict(list(sorted_final_result.items())[:5])
print(first_5)